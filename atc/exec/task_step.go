...
type TaskDelegate interface {
	...
	Finished(lager.Logger, ExitStatus, worker.PlacementStrategy, runtime.Worker)
	SelectWorker(context.Context, Pool, db.ContainerOwner, runtime.ContainerSpec, worker.Spec, worker.PlacementStrategy, time.Duration, time.Duration) (runtime.Worker, error)
...
func (step *TaskStep) run(ctx context.Context, state RunState, delegate TaskDelegate) (bool, error) {
	...
	worker, err := delegate.SelectWorker(
		ctx,
		step.workerPool,
		owner,
		containerSpec,
	...
	delegate.Finished(logger, ExitStatus(result.ExitStatus), step.strategy, worker)
